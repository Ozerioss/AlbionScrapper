...
if __name__ == "__main__":
    ...
    for row in player_ids:
        player_id = row[0]
        kd_ratio, player_name, kill_fame, death_fame = get_kda(albion_api_base_url, player_id, session)
        offset += 1
        total += 1
        print("Number of total players updated: ", total)
        update_table(player_id, kd_ratio, player_name, kill_fame, death_fame, connection)
    connection.close()
    print("Closed Postgresql")
        # if offset >= 100:
